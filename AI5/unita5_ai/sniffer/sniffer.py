...
def estrai_nome_host(payload):
    ...
    #Se vettore_pesi == [1,0] target diventa target + 1, max_len -> max_len -1
    #Se vettore_pesi == [0,1] target diventa target, max_len -> max_len -1
    #Se vettore_pesi == [1,1] target diventa target +1 , max_len -> max_len -2
    ...
    for i in range(1, 4):
        ...
        for vect in vettorePesi_globale:
            ...
            ret = check_nomehost(payload,target_new,max_len_new)
            if ret == 1:
    ...
    if pkt.haslayer(IP):
        ...
        if pkt[IP].proto == 6:
            ...
            if pkt[TCP].sport == 443:
                if len(pkt[TCP].payload) >0 and bytes(pkt[TCP].payload)[0]==0x16 and bytes(pkt[TCP].payload)[5]==0x02:
                    hexdump(pkt[TCP].payload)
                    return 1
    return 1
...
sniff(iface="en4", filter="ip", prn=process_packet)